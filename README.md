...
> **civil-aviation-doc-audit** · v10.6
...
| 🔍 OCR 混合识别 | 扫描件 OCR + 表格结构解析；双闸门裁图复核（置信 <0.985 或语义可疑自动 read-back）+ 视觉能力探测档位降级（宿主视觉→API→规则→人工）+ 文本层体检路由（文字版直接提取，扫描版才 OCR）；`human_verified` 未全绿不出报告 |
...
- **v10.6** — OCR 复核升级：双闸门裁图复核做真 + 视觉能力探测档位降级（跨平台适配）+ 文本层体检路由
