...
### Phase 1：数据底座建立（build_foundation.py）
- 文件分类（被审核/依据/排除）
- 差异化提取：非扫描 PDF 直接文本提取，扫描件 OCR，Excel 仅记录元数据
- 三层 JSON 结构（structured_rows + full_text + page_map）
- 含图 PDF 自动截图存 `_images/`
- 自动构建 link_graph.json 文档关联图谱
- human_verified 分层：非扫描件自动通过，扫描件必须人工核对
- ⛔ 闸门检查：存在未核对文档时停止，提示打开 data-editor.html

### Phase 2：人工数据核对（data-editor.html）
- 三栏界面：左侧文档树 + 右侧表格/原文/图纸三 Tab
- 表格数据可编辑，修改后保存回 JSON
- 图纸类文档可预览页面截图
- 确认按钮设置 human_verified = true
- 全部确认后方可进入审核阶段

### Phase 3：规则引擎审核（review_audit.py）
- 按 link_graph 精准加载关联文档，控制上下文量
- 铁律（L1）→ 逻辑（L2）→ 业务（L3）三级规则匹配
- 跨文档逻辑一致性检查
- 可选签字一致性检测（--check-signatures）

### Phase 4：审核报告生成
- HTML 报告（含图表、分部分项汇总、签字异常专区）
- 审核日志 JSON（完整可追溯）
- 签字异常对比图（base64 内嵌，自包含文件）
...
每份文档的 JSON 数据文件包含三层信息：

| 层级 | 字段 | 用途 |
|---|---|---|
| 第一层 | `structured_rows` | 结构化数据行，供规则引擎快速匹配 |
| 第二层 | `full_text` | 完整文本，供 LLM 深度语义审核 |
| 第三层 | `page_map` | 分页映射（页码+文本+截图路径），供人工核对定位 |

### 文档类型差异化处理

| 文档类型 | JSON | MD | 截图 | human_verified 默认 |
|---|---|---|---|---|
| 非扫描 PDF（文字型） | ✅ | ❌ | ❌ | true（自动通过） |
| 扫描件 PDF | ✅ | ❌ | ❌ | false（必须人工核对） |
| 图纸类 PDF（有图） | ✅ | ❌ | ✅（存 `_images/`） | false |
| Excel | ❌（仅 index 元数据） | ❌ | ❌ | true |
| 图片（PNG/JPG） | ✅ | ❌ | ❌ | false |

### 文档关联图谱（link_graph.json）

数据底座建立时自动构建文档关联图谱，用于审核时精准加载关联文档：

- **same_pile**：同一桩号跨文档关联
- **same_date_log**：施工日期对应施工日志
- **signer_in_roster**：签字人关联人员报审名单

### 人工核对闸门（human_verified）

- 非扫描件（Word/Excel 生成的 PDF、Excel 文件）→ 自动通过
- 扫描件/图片 → 必须在 data-editor.html 中人工核对
- 使用 `--force` 可跳过闸门（仅限测试，报告加水印）

### 签字一致性检测（可选）

启用方式：前置信息确认时选择"签字检查"，或审核时添加 `--check-signatures` 参数。

检测流程：
1. PDF 页面渲染 → 搜索签字关键词位置
2. 裁剪签字区域 → 检测手写笔迹（ink density）
3. pHash 感知哈希 + SSIM 结构相似度双指标比对
4. 首次出现作为基准样本，后续与基准比对
5. 相似度 < 70% 标记"疑似代签"，70-85% 标记"存疑"

检测结果在审核报告中以「⚠️ 签字异常核查专区」呈现，含对比图（base64 内嵌）、源文件路径、页码、建议。
...
## v7.0 升级详情（2026-07-31）

**触发需求**：v6.0 四阶段流水线基础上，数据底座需解决信息完整性、上下文溢出、签字审核缺失等问题。

**核心变更**：
1. **三层JSON数据底座**：structured_rows（规则引擎）+ full_text（LLM 审核）+ page_map（人工定位）
2. **差异化提取**：非扫描 PDF 直接文本提取（秒级），Excel 仅记元数据，取消 MD 生成
