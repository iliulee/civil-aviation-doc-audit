...
description: "v9.5 民航施工资料合规审核大师：按 MH/T 5078 规范逐条对账、OCR 识别扫描件、跨资料逻辑一致性检查、三级输出（Fatal/Sanity Check/Best Practice）、自动生成审核报告与整改通知，覆盖场道/空管/助航/弱电/供油五大专业。当用户要求审核民航施工资料、检查合规性、验证运算、识别扫描件、生成报告或整改通知时触发。"
---
...
| "人工核对"/"打开数据编辑器"/"对一下 OCR 结果" | → **场景·审核流水线** |
| "生成审核报告"/"出报告" | → **场景·审核流水线** |
...
| **G-1.9** | **步骤 4 完成 = AI 必须硬停。输出"扫描件待核实清单"后立即停止，禁止自行进入步骤 5/6/7，禁止自行把存疑项判为"没问题"** | 阻断进入步骤 5 | 打开数据编辑器，逐项核对 OCR 结果，完成后确认 | 读取到 `human_verified=true` 后自动继续 |
| **G-2** | 所有文件 human_verified 必须为 true | 阻断进入步骤 6 | 完成所有存疑项的人工核实 | 读取到 `human_verified=true` 后自动继续 |
...
步骤 5 — 阶段 2：人工核对（零 token） → 修正记录/corrections.json + corrected_data.json
...
用户在浏览器中打开 `data-editor.html` 完成所有核对操作：左图右表 → 翻页同步 → 双视图编辑 → 字段编辑记录原值/新值 → 质量告警逐条确认 → OCR 存疑项高亮 → 保存生成 `corrected_data.json` → 确认完成更新 `human_verified=true`。
...
详见 `references/CHANGELOG.md`（v1.0~v9.4 完整变更记录）。