...
def check_inference():
    ...
    has_all_fields = all(
        all(k in r for k in ("type", "condition", "formula", "base_confidence", "cascade_penalty"))
        for r in rule_list
    )
    check("G", "每条规则含 type/condition/formula/base_confidence/cascade_penalty", has_all_fields)
