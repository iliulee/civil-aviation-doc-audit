...
description: "v9.7.1 民航施工资料合规审核大师：按 MH/T 5078 规范逐条对账、OCR 识别扫描件、跨资料逻辑一致性检查、三级输出（Fatal/Sanity Check/Best Practice）、自动生成审核报告与整改通知，覆盖场道/空管/助航/弱电/供油五大专业。当用户要求审核民航施工资料、检查合规性、验证运算、识别扫描件、生成报告或整改通知时触发。"
---

# 民航建设施工资料合规审核大师 v9.7.1
...
| **G-1.5** | 项目文件夹 `数据底座/` 下必须存在：数据核对编辑器.html、项目总览.html、tokens.css、pdf.min.js、pdf.worker.min.js | 阻断进入步骤 5 | — | — |
...
| 2 | `nature` | 资料形式：电子版/扫描件/混合/图纸/**扫描转化电子文档** | — |
...
详见 `rules/registry.json`。其中铁律 9/12/13/14 由 L2 逻辑规则承载，L1-IRON 实际 17 条规则文件。
...
```
步骤 4 — 阶段 1：建数据底座（全自动） → 数据底座/（JSON三层+index.json+项目总览.html）
  推断：build_foundation.py 自动读取 rules/inference_rules.json 生成推荐值
  对比清单：增量路径展示已有文件 vs 新增文件清单，用户确认后执行
  闸门：ocr_status = "completed" + G-1.5 模板已复制 → G-1.9 硬停
...
- **任务分两档（v9.7.1）**：`scope=table`（施工部位/施工日期等整表同值字段，落库写整表）；`scope=row`（实长/电流/充盈系数等行级数值字段，落库只写该桩号所在行）。「整行」核对项（表头不可靠）不进视觉复核，留给 Chat-Verify 人工核对
