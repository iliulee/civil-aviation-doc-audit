...
# 民航建设施工资料合规审核大师（civil-aviation-doc-audit）v1.7
...
| 3 | Tesseract OCR | 自动下载安装（~50MB），含中文语言包 |
| 4 | 系统 PATH | 自动配置，无需手动 |
| 5 | 验证 | 逐一检查所有组件可用 |
...
### 第 0 步：审核前置信息收集（新增 v1.7，v1.7.1 强化）

> **设计意图**：审核前必须了解资料的背景上下文，避免因信息不对称导致误判。v1.7.1 新增文件分类确认环节，明确区分"被审资料"和"依据文件"。
...
**目的**：把用户提供的所有文件分两类——哪些是审的、哪些是依据。

**执行方式**：列出用户提供的所有文件，逐项标注分类，请用户确认：
...
**统一交付物（HTML）**：
> v1.7 起，审核报告、整改通知书、合规性检查清单三者合并为一个 HTML 文件，方便阅读和分发。HTML 报告必须包含以下全部章节：
...
| 四、数据质量审查 | 铁律10四类检测结果 | 审核报告 |
| 五、逻辑一致性专项检查 | 铁律9八项子检查 | 审核报告 |
| 六、不符合项清单汇总 | 所有不符合项表格（含置信度） | 合规性检查清单 |
| 七、审核结论与整改要求 | 总体结论 + P1/P2/P3分级整改 | 整改通知书 |
| 八、OCR待核实清单（铁律20） | 低置信度数据项，供人工逐项确认 | 新增 |
| 九、审核日志 | 审核时间、依据、OCR置信度、铁律触发 | 审核日志 |
