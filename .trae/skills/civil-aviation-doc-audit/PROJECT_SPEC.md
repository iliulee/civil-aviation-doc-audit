# PROJECT_SPEC — 民航施工资料合规审核 Skill v10.6

> 文档版本：v3.5 | 创建日期：2026-07-29 | 最后更新：2026-08-29 | 状态：v10.6（OCR 复核升级 + 跨平台视觉调度 + 文本层体检路由）已实施
>
> 本文档是项目的唯一权威规格说明，覆盖全部已有功能（v5.0）、v6.0 四阶段流水线 + 数据底座、v6.1 规则管理子系统、v7.0 三层JSON数据底座 + 文档关联图谱 + 签字一致性检测、v7.1 data-editor 精密仪表盘升级、v7.2 数据底座基础能力增强、v8.0 OCR 策略重定位、v8.1 双模式运行架构、v8.2/8.3/8.4/8.5/8.6 强制闸门与增量路径修复、v8.7 OCR 全流程加固 + 模板目录重构 + 用户体验全面升级、v8.8 PaddleOCR 引擎升级、v9.0 OCR 模块重构（RapidOCR + 手写体前置 VLM 路由）、v9.1（RapidTable 表格结构识别 + 方式二对话式核对四条纪律）、v9.4（表格式样导入导出 + 苹果浅色界面优化 + SKILL.md 内部路由重构）、v9.5（审核流水线步骤 1~7 线性编号 + 前置信息表迁移共享基础设施 + 推断值生成规则 + 闸门用户动作说明 + 规则管理步骤化工作流）、v9.7（AI 视觉复核链贯通 + 协议平台无关化）、v9.7.1（视觉复核任务行级/表级分流）、v10.0（资料员工作台九页合一 + 版本号统一）、v10.2（Excel/docx 建数据底座链路修复：解析不截断/row_index 行级定位/列语义对齐/质检误报修复）、v10.3（材料/合格证数据链沉淀：材料类信号路由/OCR 零产出判定/材料 schema 契约/合格证提取与台账落库/S-04 追溯链/LG-110 新规则/规则注册表与校验器对齐）、v10.4（规则→审核→报告全链路贯通：LG-110 触发词修正消灭静默失效/审核期 S-04 重算/规则执行统计 matched_docs+hits/报告生成建模渲染分离 report_builder/三层报告结构/整改建议列渲染/页脚版本号单一真相源/verify_report 合格证台账对账/规则覆盖扫描工具）、v10.5（无规则覆盖运行时闸门：build_unguarded_doc_types 侦测受审文档中无 active 规则覆盖的 doc_type/报告「八、无规则覆盖提醒」节/SKILL.md 强制提醒协议/scan_rule_coverage 口径校准复用同一判定/IR-010 IR-011 LG-905 零交集规则触发词修复/test_unguarded_doc_types 回归套件）。
...
```json
{
  ...
  // ===== 三层结构（v7.0+） =====
  // 行级定位键 row_index 三链路统一落实（v10.2）：
  //   扫描/PDF 行 → 来源页+行号（page/line_no）+ row_index；
  //   Excel 行    → row_index = sheet 内物理行号（可落回原单元格）+ 英文标准槽位键（pile_no/actual_length… 列语义对齐）；
  //   docx 行     → row_index = 表内数据行序号（table=表序号）。
...
把原本分散的 9 个审核 HTML 入口（审核、数据核对、规则管理、反馈等）合并为**单一工作台**。核心目标：**一次加载数据、多模块共享**，避免跨页重复读盘、来回开窗口。

既有的 `data-editor.html`（177KB，v1）**原样保留零改动**，由工作台外壳跳转打开；只有新版本才用 postMessage 并入。

### 12.2 七个模块

| 模块 | 功能 |
|:---|:---|
| 1. 项目总览 | 加载 `index.json`，文档统计 + 进度总况 |
| 2. 数据核对 | 内嵌 data-editor，逐条核对 OCR 结果 |
| 3. 资料进度看板 | 8 节点轴「开检隐分竣交档」+ SortableJS 拖拽流转，覆盖层落 localStorage |
| 4. 台账三本 | 检验批/隐蔽/混凝土等台账管理 + 计数 |
| 5. 数据概览导出 | SheetJS/xlsx 导出 |
| 6. 整改销号 | 问题登记→销号闭环循环 |
| 7. 规则与反馈 | 内嵌 8765 规则面板 + 存活探测降级提示（替代旧 rule-manager.bat 单独入口） |

顶部导航 + hash 路由 + 动态 import 按需加载，模块独立拆分。

### 12.3 技术栈与数据层

- **技术栈**：Vite（构建）+ SortableJS（看板拖拽）+ SheetJS/xlsx（导出）+ ECharts（离线图表）+ IndexedDB（句柄持久化）。禁止手写拖拽/导出，禁止自造轮子替代已选开源件。
- **数据层**：一次加载 `index.json`，`_index` 内存共享；IndexedDB 存 `FileSystemDirectoryHandle` 实现「一键恢复上次项目」；写入用**原子写 + 自动备份**（备份至 `backups/`），不迁 SQLite。
- **双模加载**：HTTP fetch（部署/开发）/ FileSystem Access API（本地文件系统）。

### 12.4 部署与启动

- 前端构建产物 `dist/` 部署至 `资料员工作台/` 目录，含 `index.html`、`index.json`、`assets/`。
- **一键启动 `启动工作台.bat`**：同时拉起规则 API(`:8765`) + 前端静态服务(`:8909`) + 打开浏览器。批处理**纯 ASCII 编码**落盘，避免 cmd 代码页(GBK)乱码。
- 规则服务参数必须用 `--rules-dir`（不可用 `--rules`），否则端口未绑定就退出、规则面板起不来。

### 12.5 测试与验收入口

- `scripts/test_workbench.py`：工作台结构断言（依赖/构建/manifest/数据层/外壳/路由），已注册进 `run_all_tests.py`。
- `verify_plan_v2.py` 新增「工作台 v10：部署管线接入」11 项。
- 交付验收：`run_all_tests` 全绿 + DOC-001 golden 回归 + 双端（项目版/安装版）构建产物哈希一致 + CHANGELOG v10.0 条目。

---

*文档结束 — v10.6 OCR 复核升级 + 跨平台视觉调度 + 文本层体检路由已实施，v10.5 无规则覆盖运行时闸门已实施，v10.4 规则→审核→报告全链路贯通 + 报告三层结构已实施，v10.3 材料/合格证数据链沉淀已实施，v10.2 Excel/docx 建数据底座链路修复已实施，v10.0 资料员工作台已实施，v7.1 已实施，v7.0 三层JSON数据底座与文档关联图谱已实施，v6.1 规则管理子系统已实施，v6.1.1 审核闸门与字段别名映射修复已实施，全部测试通过*
