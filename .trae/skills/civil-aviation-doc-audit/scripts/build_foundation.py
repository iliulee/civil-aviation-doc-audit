...
def call_extract_docx(file_path: Path, text_out: Path) -> Dict[str, Any]:
    ...
    返回 {"text", "engine", "confidence", "items": [], "page_count": ...}。
    v9.5：返回 page_count = 表格数（扫描转化电子文档每页≈一张表），
    供 main() 修正 pages，避免 docx 页码提取失真为 1。
    """
    ...
    try:
        ...
        return {"text": "", "engine": "docx", "confidence": 0.0, "items": [], "page_count": 1}
    try:
        ...
        _docx_page_count = len(doc.tables) or 1
        text = "\n".join(parts)
    except Exception as e:
...
# ========== v9.5: docx 表格结构解析下沉（吸收原外部补丁 enrich_docx.py） ==========

_DOCX_NUMERIC_FIELDS = [
    "design_length", "diameter", "bottom_elev", "top_elev", "actual_length",
    "current", "re_penetration", "volume", "filling_coeff", "verticality",
]

_DOCX_TIME_FIELDS = ["sink_start", "sink_end", "pull_start", "pull_end"]

_DOCX_ALL_FIELDS = _DOCX_NUMERIC_FIELDS + _DOCX_TIME_FIELDS


def _docx_clean_cell(t: Any) -> str:
    """清洗 docx 单元格文本：清全角空格/\xa0；Python None 与 'None'/'null'/'nan' 统一空串。"""
    if t is None:
        return ""
    s = str(t).replace("\u3000", "").replace("\xa0", " ").strip()
    if s.lower() in ("none", "null", "nan"):
        return ""
    return s


def _docx_sub_header_label(cell_text: str) -> Optional[str]:
    """按表头标签识别列语义（与 enrich_docx.py 一致的字段映射）。"""
    t = _docx_clean_cell(cell_text)
    if not t:
        return None
    if re.search(r"序号|桩\s*号|桩号", t):
        return "pile_no"
    if re.search(r"桩\s*长", t) and not re.search(r"实际桩长|实长", t):
        return "design_length"
    if re.search(r"桩\s*径", t):
        return "diameter"
    if "桩底" in t:
        return "bottom_elev"
    if "桩顶" in t:
        return "top_elev"
    if "实长" in t or "实际桩长" in t:
        return "actual_length"
    if re.search(r"密实|电流", t):
        return "current"
    if "反插" in t:
    ...
    if "灌入" in t:
        return "volume"
    if "充盈" in t:
        return "filling_coeff"
    if "竖直" in t:
        return "verticality"
    return None


def _docx_try_float(s: str) -> Optional[float]:
    """宽松数值提取（OCR 乱码容错），返回 None 表示不可解析。"""
    if not s:
        return None
    s = str(s).replace("，", ".").replace(",", ".").replace("o", "0").replace("O", "0")
    m = re.search(r"(\d+(?:\.\d+)?)", s)
...
def parse_docx_table_sheets(file_path: Path) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], str]:
    ...
    按表头标签驱动解析「扫描转化电子文档」内的施工记录表（如碎石桩记录），
    处理合并单元格与变列数，直接产出结构化 rows 与存疑清单。

    返回 (structured_rows, suspects, detected_doc_type)：
      - structured_rows: [{table, loc, date_raw, pile_no, design_length, ..., pull_end}]
      - suspects: [{table, pile_no?, field, raw, reason}]（部位/日期/数值不可解析/整表表头不可靠）
      - detected_doc_type: 依据表头 schema 判定，如 "碎石桩施工记录"；无法判定返回 ""
    """
    try:
        from docx import Document
    except ImportError as e:
    ...
    for ti, tab in enumerate(doc.tables):
        ...
        # 表头 meta：部位/日期/材料
        if len(rows) > 2:
            r2 = [_docx_clean_cell(c.text) for c in rows[2].cells]
            if len(r2) > 2 and not re.search(r"序号|桩|沉管|密实|充盈|竖直", r2[2]):
                loc = r2[2]
            for j in range(len(r2)):
                if r2[j].replace(" ", "") == "施工日期":
                    for k in range(j + 1, min(j + 4, len(r2))):
                        if re.search(r"\d", r2[k]):
                            date_raw = r2[k]
                            break
                    break
        if len(rows) > 3:
            mat = _docx_clean_cell(rows[3].cells[0].text)
        ...
        for ri in range(3, len(rows)):
            cells = [_docx_clean_cell(c.text) for c in rows[ri].cells]
            joined = " ".join(cells)
            if (re.search(r"序号|桩\s*号|设计桩|沉管|密实|充盈|竖直|桩\s*径|桩底|实\s*长", joined)
                    and "原材料" not in joined and ri < 8):
                header_rows.append(ri)
                seen_header = True
                continue
            if seen_header and ri > 0:
                if re.search(r"\d", joined) and not re.search(r"施工员|质检员|技术负责人|监理单位|交勋|^牟|项目专业", joined):
                    data_rows.append((ri, cells))
                elif joined.strip() and not re.search(r"\d", joined):
                    seen_header = False

        if not header_rows:
            tables.append({"idx": ti, "loc": loc, "date_raw": date_raw, "mat_desc": mat,
                           "header_ok": False, "rows": []})
            continue

        # 最细表头行 + 父分组行（沉管/拔管）
        detail_ri = header_rows[-1]
        parent_ri = header_rows[-2] if len(header_rows) >= 2 else detail_ri
        detail_cells = [_docx_clean_cell(c.text) for c in rows[detail_ri].cells]
        parent_cells = [_docx_clean_cell(c.text) for c in rows[parent_ri].cells]

        # 父分组：沉管/拔管 grid index 集
        parent_sem: List[Tuple[str, int]] = []
        for i, t in enumerate(parent_cells):
            t2 = t.replace(" ", "")
            if "沉管" in t2:
                parent_sem.append(("sink", i))
            elif "拔管" in t2:
                parent_sem.append(("pull", i))

        grid_field: Dict[int, str] = {}
        for i, t in enumerate(detail_cells):
            f = _docx_sub_header_label(t)
            if f:
                grid_field[i] = f
            elif t.replace(" ", "") in ("起", "止"):
                which = "sink"
                sink_idx = [x for k, x in parent_sem if k == "sink"]
                pull_idx = [x for k, x in parent_sem if k == "pull"]
                if sink_idx and i <= max(sink_idx):
                    which = "sink"
                elif pull_idx:
                    which = "pull"
                elif i <= 5:
                    ...
                    which = "pull"
                grid_field[i] = f"{which}_start" if t.replace(" ", "") == "起" else f"{which}_end"

        parsed: List[Dict[str, Any]] = []
        for ri, cells in data_rows:
            rec: Dict[str, Any] = {"table": ti, "loc": loc, "date_raw": date_raw, "mat_desc": mat}
            for i, val in enumerate(cells):
                f = grid_field.get(i)
                if f:
                    rec.setdefault(f, val)
                if i == 0:
                    rec["pile_no"] = val
            parsed.append(rec)
        tables.append({"idx": ti, "loc": loc, "date_raw": date_raw, "mat_desc": mat,
                       "header_ok": len(grid_field) >= 10, "grid_field": grid_field,
                       "rows": parsed})
    ...
    for t in tables:
        if t["header_ok"]:
            header_ok_count += 1
        if not re.search(r"[\u4e00-\u9fff0-9]", t["loc"] or ""):
            suspects.append({"table": t["idx"], "field": "施工部位", "raw": t["loc"], "reason": "部位为空白"})
        elif not (re.search(r"碎石", t["loc"]) or re.search(r"桩", t["loc"])):
            suspects.append({"table": t["idx"], "field": "施工部位", "raw": t["loc"], "reason": "部位疑似OCR乱码，无法对应到『碎石桩x区』"})
        if t["date_raw"]:
            if not re.fullmatch(r"2026[\.,、/].*", t["date_raw"].replace(" ", "")):
        ...
        for r in t["rows"]:
            ...
            if not t["header_ok"]:
                suspects.append({"table": t["idx"], "pile_no": row["pile_no"], "field": "整行",
                                 "raw": "", "reason": "该表表头解析不可靠，整行数值字段需人工核对"})
            for fld in _DOCX_NUMERIC_FIELDS:
                _cur_raw = r.get(fld, "")  # 缺键/None 统一取空串，杜绝 raw/reason 取值不一致（『None』误导）
                v = _docx_try_float(_cur_raw)
                if v is None:
                    suspects.append({"table": t["idx"], "pile_no": row["pile_no"], "field": fld,
                                     "raw": _cur_raw, "reason": f"数值不可解析:『{_cur_raw}』"})

    # 表头 schema 判定 doc_type（多数表含桩号/桩长/桩径等碎石桩列 → 碎石桩施工记录）
    detected_doc_type = ""
    if tables and header_ok_count >= max(1, len(tables) // 2):
        first_ok = next((t for t in tables if t["header_ok"]), None)
        if first_ok:
            fields = set(first_ok.get("grid_field", {}).values())
            if "pile_no" in fields and any(f in fields for f in ("design_length", "diameter", "actual_length", "filling_coeff", "verticality")):
                detected_doc_type = "碎石桩施工记录"

    return structured, suspects, detected_doc_type


def _sanitize_rows_none(rows: List[Dict[str, Any]]) -> None:
    """v9.5：行内所有值为 Python None / 'None' 的字段统一改为 ''，杜绝 None 污染 JSON。"""
    for r in rows:
...
def build_page_map(file_path: Path, is_scanned: bool, ocr_text: str, method: str) -> List[Dict[str, Any]]:
    ...
    if method == "pymupdf" and HAS_PYMUPDF:
        try:
            ...
            page_map = [{"page": 1, "text": ocr_text, "image_ref": ""}]
    else:
        page_map = [{"page": 1, "text": ocr_text, "image_ref": ""}]
    return page_map
...
def main() -> int:
    ...
    for rel in rel_files:
      try:
        ...
        docx_suspects: List[Dict[str, Any]] = []  # v9.5: docx 表格解析存疑清单
        ...
        if method == "ocr":
            ...
            ocr_confidence = result.get("confidence", 1.0)
            # v9.5 下沉：docx 直接按表格结构解析（替代原外部补丁 enrich_docx.py），
            # 产出结构化 rows + 存疑清单，并按表头 schema 判定 doc_type（如碎石桩施工记录）。
            _docx_rows, docx_suspects, _docx_detected = parse_docx_table_sheets(abs_path)
            if _docx_rows:
                _sanitize_rows_none(_docx_rows)
                rows = _docx_rows
                ocr_confidence = 0.4 if docx_suspects else 1.0
                ocr_status = "needs_review" if docx_suspects else "completed"
            # 页码修正：扫描转化电子文档按表格数估算页数（27 张表 ≈ 27 页扫描件）
            _docx_pages = result.get("page_count") or 0
            if _docx_pages > 1 and (pages <= 1 or (preconditions.get("nature") == "扫描转化电子文档")):
                pages = _docx_pages
            # 表头 schema 命中 → 回写 doc_type/subcategory，并把数据文件重定向到正确子目录，
            # 从根上解决「分类与物理路径不一致」（原 enrich_docx.py 只改 index 不搬文件）
            if _docx_detected and _docx_detected != doc_type:
                doc_type = _docx_detected
                subcategory = _docx_detected
                classification_source = "docx_table_schema_match"
                ...
                confusion_file = folder / f"{base_name}_confusion.json"
                text_file = folder / f"{base_name}_raw.txt"
                print(f"  [docx schema] 判定 {_docx_detected}，数据文件归位到 {folder.name}", file=sys.stderr)
                # call_extract_docx 在重定向前已把文本写到旧路径，这里把 _raw.txt 也落一份到新目录，保持路径自洽
                if ocr_text and not text_file.exists():
        ...
        structured = {
            ...
            "table_struct": table_struct_res if table_struct_res is not None else "",
            ...
            # v9.5: docx 表格解析存疑清单（供阶段2 人工核对/Chat-Verify 读取）
            "pending_verification": docx_suspects,
        }
        ...
        confusion_result = {"suspects": []}  # unsupported 路径不跑检测，预置空结构防未定义引用
                ...
                for row_idx_str, inferred_fields in row_inferred.items():
                    try:
                        ...
                        if 0 <= row_idx < len(rows):
                            # 铁律：文本建议值（suggested_only）只建议不入库，
                            # 不污染 structured_rows.inferred，也不参与后续审核判定。
                            persistable = {
                                k: v for k, v in inferred_fields.items()
                                if isinstance(v, dict) and not v.get("suggested_only")
                            }
                            rows[row_idx]["inferred"] = persistable
        ...
        # ===== v9.7 A3：扫描转化电子文档 → 自动生成视觉复核任务 =====
        # 断链修复：confusion（易混字）+ pending（表级乱码）两套存疑清单从未接进
        # verify_fields 复核器。此处自动 prepare（docx 内嵌图裁整页 + 任务清单），
        # 宿主 AI（TRAE / WorkBuddy / CodeBuddy 等任意平台）按协议读图回写结果。
        # 触发条件对齐文档语义：仅 nature=扫描转化电子文档（图片型 docx）——
        # 原生电子 docx 的表格不是内嵌图，裁不了图，不触发。
        verify_tasks_rel = ""
        _nature = (preconditions.get("nature") or "").strip()
        _is_scan_converted = _nature == "扫描转化电子文档" and method == "docx"
        if _is_scan_converted and (confusion_suspects or structured.get("pending_verification")):
            try:
                from verify_fields import prepare_verify_tasks
                verify_dir = folder / "verify_output"
                tasks_result = prepare_verify_tasks(
                    str(abs_path), confusion_result, structured, str(verify_dir))
                if tasks_result.get("status") == "prepared":
                    verify_tasks_rel = str(verify_dir.relative_to(out_base)).replace("\\", "/")
                    print(f"  [复核] 已生成 {tasks_result['total_tasks']} 条视觉复核任务 → {verify_tasks_rel}/verify_tasks.json",
                          file=sys.stderr)
            except Exception as e:
                print(f"  [!] 视觉复核任务生成失败（不阻塞底座构建）: {e}", file=sys.stderr)
        ...
        doc = {
            ...
            "verify_tasks_file": verify_tasks_rel or None,  # v9.7: 视觉复核任务清单（有存疑时生成）
            ...
            # v9.5: docx 表格解析存疑项落 index，供阶段2/Chat-Verify 读取
            "pending_verification": docx_suspects,
            # v9.5: 状态注按提取方式区分——只有 docx（扫描转化电子文档）才标扫描转化，
            # 其余文档（xlsx/pdf/text）不误标，避免状态注与真实提取方式不一致。
            "summarized_status_note": (
                f"扫描转化电子文档，{len(docx_suspects)} 条存疑项待人工核对（部位/日期/数值）"
                if method == "docx" and docx_suspects
                else ("扫描转化电子文档" if method == "docx" else f"{doc_type}，共 {len(rows)} 行")
            ),
        }
