...
if require_lookup() is not None:
    ...
    # 2c. 多部分规范在 obsidian 层必须按分部精确 glob，不得 5078.2~6 误归到 5078.1
    #     （references 层本就有 5078.2 的一致性映射，命中属正确行为；此处传
    #       references_dir=None 隔离，单独验证 obsidian 层 _glob_vault 按分部定位）
    for part_no, expect_part in [("5078.2", "第2部分"), ("5078.3", "第3部分"),
                                 ("5078.6", "第6部分")]:
        hit_part = ls.lookup_source(f"MH/T {part_no}", content_snippet="资料",
                                    references_dir=None,
                                    vault_dir=str(VAULT_DIR))
        fname = str(hit_part.get("file", ""))
        check(f"2c {part_no} obsidian 命中分部文件", hit_part.get("found") is True
              and hit_part.get("source") == "obsidian" and expect_part in fname,
              f"found={hit_part.get('found')} source={hit_part.get('source')} file={fname}")
