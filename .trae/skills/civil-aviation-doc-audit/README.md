...
│
├── rules/                            # 【v6.0】规则文件库（91 条）
│   ├── L1-iron/                      # L1 铁律（17 条）
│   ├── L2-logic/                     # L2 逻辑一致性（71 条，含 IR-012/013/014 三条迁移自原铁律的几何/合计/多参数联检规则）
...
91 条规则三层分级（L1铁律/L2逻辑一致性/L3业务合理性）+ 跨单位对照特殊作用域，形式化 JSON 存储，支持可视化管理、反馈闭环、LLM 自成长。

### 核心能力

| 能力 | 说明 |
|------|------|
| **三层规则分级**（v6.0） | L1铁律(17)/L2逻辑一致性(71)/L3业务合理性(3)/跨单位对照(18)，共 91 条 |
...
### 规则管理 API（rule_admin.py）

规则管理：GET/POST/PUT/DELETE `/api/rules`、`/api/rules/{id}/transition`、`/api/rules/{id}/confirm`、`/api/rules/{id}/test`
反馈管理：GET/POST `/api/feedbacks`、POST `/api/feedbacks/analyze`
反思与自成长：GET `/api/reflections`、POST `/api/reflections/trigger`、GET/POST `/api/incubator/{id}/promote|reject`
